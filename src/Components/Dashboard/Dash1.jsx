...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('bed1d'); // Replace with your actual backend route
        setAdmissionData([{ x: 'This Week', y: response.data.admissionStatistics.thisWeek }, { x: 'This Month', y: response.data.admissionStatistics.thisMonth }]);
      } catch (error) {
  ...
  // Check if admissionData is an array before rendering
  if (!Array.isArray(admissionData)) {
    console.error('Admission data is not an array:', admissionData);
  ...
  useEffect(() => {
    ...
    const staticData = {
      ...
      links: [
        { source: "ICU", target: "ER", value: 10 },
        { source: "ER", target: "General", value: 15 },
